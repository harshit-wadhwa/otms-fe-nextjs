...
export default function TestReviewPage() {
  ...
  useEffect(() => {
    const fetchTestResult = async () => {
      ...
      setError('');
      try {
        const response = await apiClient.get<TestResult>(`/student/test-result/${testId}`);
    ...
    <ProtectedRoute>
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 py-8">
        ...
        <div className="mb-4 max-w-7xl mx-auto px-4">
          <Breadcrumb
            items={[
              { label: 'Dashboard', href: user?.role === 'teacher' ? '/dashboard' : '/student/dashboard' },
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 p-8 mb-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 text-sm text-gray-600">
              <div>Duration: {formatTime(testResult.test_duration)}</div>
              <div>Submitted: {formatDate(testResult.submitted_at)}</div>
            </div>
          </div>
                  ...
                  return (
                    <div key={idx} className={optionClass}>
                      <div className="flex items-center">
                        ...
                        {isStudentAnswer && !isCorrect && <span className="ml-auto text-sm font-semibold">✗ Your Answer</span>}
                      </div>
                    </div>
                  );
                })}
